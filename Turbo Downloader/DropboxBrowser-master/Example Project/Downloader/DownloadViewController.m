...
- (void)viewDidLoad
{
    ...
    self.bannerView2.adUnitID = @"ca-app-pub-3940256099942544/2934735716";
    self.bannerView2.rootViewController = self;
    GADRequest *request4 = [GADRequest request];
    // Requests test ads on devices you specify. Your test device ID is printed to the console when
    // an ad request is made. GADBannerView automatically returns test ads when running on a
    // simulator.
    request4.testDevices = @[
                             @"2077ef9a63d2b398840261c8221a0c9a"  // Eric's iPod Touch
                             ];
    [self.bannerView2 loadRequest:request4];
...
-(void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession *)session{
    DBBAppDelegate *appDelegate = [UIApplication sharedApplication].delegate;
